...
namespace pxt {
    ...
    DescriptorAllocatorGrowable::DescriptorAllocatorGrowable(Context& context, const uint32_t maxSets,
                                                             ...
                                                             const float growthFactor, const uint32_t maxPools)
    ...
    Shared<DescriptorPool> DescriptorAllocatorGrowable::createPool(uint32_t setCount,
                                                                   std::span<PoolSizeRatio> poolRatios) const {
        ...
        Shared<DescriptorPool> pool =
            DescriptorPool::Builder(m_context).addPoolSizes(poolSizes).setMaxSets(setCount).build();

        return pool;
    }

    void DescriptorAllocatorGrowable::allocate(VkDescriptorSetLayout descriptorSetLayout,
