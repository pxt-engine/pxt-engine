...
namespace pxt {
    VulkanSampler::VulkanSampler(Context& context, VkFilter filter, VkSamplerAddressMode addressMode,
                                 bool unnormalizedCoordinates, VkSamplerMipmapMode mipMapMode,
    ...
    VulkanSampler::VulkanSampler(Context& context, const VkSamplerCreateInfo& samplerCreateInfo)
        : m_context(context), m_info(samplerCreateInfo) {
        m_sampler = context.createSampler(m_info);
    }

    Unique<VulkanSampler> VulkanSampler::createSimpleLinearSampler(Context& context, bool unnormalizedCoordinates) {
        return createUnique<VulkanSampler>(context, VK_FILTER_LINEAR, VK_SAMPLER_ADDRESS_MODE_REPEAT,
                                           unnormalizedCoordinates, VK_SAMPLER_MIPMAP_MODE_LINEAR);
    }

    Unique<VulkanSampler> VulkanSampler::createSimpleNearestSampler(Context& context, bool unnormalizedCoordinates) {
        return createUnique<VulkanSampler>(context, VK_FILTER_LINEAR, VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE,
                                           unnormalizedCoordinates, VK_SAMPLER_MIPMAP_MODE_NEAREST);
    }
