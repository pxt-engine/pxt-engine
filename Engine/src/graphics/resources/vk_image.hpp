...
namespace pxt {
    ...
    public:
        ...
        VulkanImage(Context& context, const VkImageCreateInfo& imageInfo,
        ...
        uint32_t getWidth() override { return m_info.width; }

        uint32_t getHeight() override { return m_info.height; }

        uint16_t getChannels() override { return m_info.channels; }

        ImageFormat getFormat() override { return m_info.format; }

        Type getType() const override { return Type::Image; }

        VkDescriptorImageInfo getImageInfo(bool useOwnSampler = true) {
            PXT_ASSERT(useOwnSampler == false || m_sampler != nullptr,
                       "Trying to get image info with sampler but sampler is null!");
            return VkDescriptorImageInfo{.sampler = useOwnSampler ? m_sampler->getHandle() : VK_NULL_HANDLE,
                                         .imageView = m_imageView,
                                         .imageLayout = m_currentLayout};
        }

        VkExtent2D getExtent() const { return {m_info.width, m_info.height}; }

        float getAspectRatio() const { return static_cast<float>(m_info.width) / static_cast<float>(m_info.height); }
         ...
         *
         * This function transitions the layout of an image, which is required when changing the way the image is
         * accessed.
         *
         ...
         * @param sourceStage The source pipeline stage. If not specified, it will be set to
         * VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, which is less efficient.
         * @param destinationStage The destination pipeline stage. If not specified, it will be set to
         * VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, which is less efficient.
         */
        void
        transitionImageLayoutSingleTimeCmd(VkImageLayout newLayout,
    ...
    protected:
        ...
        Shared<VulkanSampler> m_sampler =
            nullptr; // an abstraction (and tool) to help fragment shader pick the right color and
