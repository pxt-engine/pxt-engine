...
namespace pxt {
    ...
    Texture2D::Texture2D(Context& context, const ImageInfo& info, const std::span<uint8_t> buffer)
    ...
    void Texture2D::createTextureImage(const std::span<uint8_t> buffer) {
        VkDeviceSize imageSize =
            m_info.width * m_info.height * m_info.channels * getChannelBytePerPixelForFormat(m_info.format);
        // create a staging buffer visible to the host and copy the pixels to it
        Unique<VulkanBuffer> stagingBuffer =
            createUnique<VulkanBuffer>(m_context, imageSize, 1, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
                                       VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);

        stagingBuffer->map(imageSize);
        stagingBuffer->writeToBuffer(buffer.data(), imageSize, 0);
        stagingBuffer->unmap();

        // create an empty vkImage
        createImage(m_info.width, m_info.height, VK_IMAGE_TILING_OPTIMAL,
        ...
        // we now change the layout of the image for better destination copy performance
        // (VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL)
        transitionImageLayoutSingleTimeCmd(VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,
                                           VK_PIPELINE_STAGE_TRANSFER_BIT);

        // then we copy the contents of the image (that were inside the stagingBuffer) into the vkImage
        m_context.copyBufferToImage(stagingBuffer->getBuffer(), m_vkImage, m_info.width, m_info.height);

        // finally, we change the image layout again to be accessed from the shaders
        // (VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL)
        transitionImageLayoutSingleTimeCmd(VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_PIPELINE_STAGE_TRANSFER_BIT,
                                           VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT);
    }

    void Texture2D::createImage(uint32_t width, uint32_t height, VkImageTiling tiling, VkImageUsageFlags usage,
    ...
    void Texture2D::createTextureSampler() {
        ...
        VkBool32 useUnnormalizedCoordinates =
            (m_info.flags & ImageFlags::UnnormalizedCoordinates) != ImageFlags::None ? VK_TRUE : VK_FALSE;
        ...
        samplerInfo.addressModeU =
            useUnnormalizedCoordinates ? VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE : VK_SAMPLER_ADDRESS_MODE_REPEAT;
        ...
        // VK_FALSE: Uses normalized texture coordinates [0,1) range, useful for giving same coords to different
        // resolution textures. VK_TRUE: Uses actual texel coordinates [0, texWidth) and [0, texHeight)]
        ...
        samplerInfo.mipmapMode =
            useUnnormalizedCoordinates ? VK_SAMPLER_MIPMAP_MODE_NEAREST : VK_SAMPLER_MIPMAP_MODE_LINEAR;
