...
namespace pxt {
    ...
    std::string VulkanShader::preprocessShader(const std::string_view& fileName, const std::string& source,
    ...
    std::string VulkanShader::compileToAssembly(const std::string_view& fileName, const std::string& source,
                                                shaderc_shader_kind shaderKind) {
    ...
    std::vector<uint32_t> VulkanShader::compileFile(const std::string_view& fileName, const std::string& source,
                                                    shaderc_shader_kind shaderKind) {
    ...
    public:
        ...
        size_t find_first_not_of(const string_piece& to_search, size_t pos = 0) const {
            if (pos >= size()) {
                return npos;
            }
        ...
        size_t find_last_not_of(const string_piece& to_search, size_t pos = npos) const {
            if (empty())
                return npos;
            if (pos >= size()) {
                pos = size();
        ...
        string_piece lstrip(const string_piece& chars_to_strip) const {
            ...
            for (; begin < end_; ++begin)
                if (chars_to_strip.find_first_of(*begin) == npos)
                    break;
            if (begin >= end_)
                return string_piece();
        ...
        string_piece rstrip(const string_piece& chars_to_strip) const {
            iterator end = end_;
            for (; begin_ < end; --end)
                if (chars_to_strip.find_first_of(*(end - 1)) == npos)
                    break;
            if (begin_ >= end)
                return string_piece();
        ...
        bool operator!=(const string_piece& other) const { return !operator==(other); }
        ...
        size_t find(const string_piece& substr, size_t pos = 0) const {
            ...
            if (substr.empty())
                return 0;
            for (auto it = begin_ + pos; end() - it >= static_cast<decltype(end() - it)>(substr.size()); ++it) {
                if (string_piece(it, end()).starts_with(substr))
                    return it - begin_;
            }
            return npos;
        }
        ...
        std::vector<string_piece> get_fields(char delimiter, bool keep_delimiter = false) const {
    ...
    inline bool operator==(const char* first, const string_piece second) { return second == first; }

    inline bool operator!=(const char* first, const string_piece second) { return !operator==(first, second); }

    // Returns "" if path is empty or ends in '/'.  Otherwise, returns "/".
    std::string MaybeSlash(const string_piece& path) { return (path.empty() || path.back() == '/') ? "" : "/"; }

    std::string VulkanShader::FileFinder::findReadableFilepath(const std::string& filename) const {
    ...
    std::string VulkanShader::FileFinder::findRelativeReadableFilepath(const std::string& requesting_file,
        ...
        static const auto for_reading = std::ios_base::in;
        std::filebuf opener;
        const std::string relative_filename = dir_name.str() + MaybeSlash(dir_name) + filename;
    ...
    shaderc_include_result* VulkanShader::FileIncluder::GetInclude(const char* requested_source,
        ...
        const auto ReadFile = [](const std::string& input_file_name, std::vector<char>* input_data) -> bool {
        ...
        return new shaderc_include_result{new_file_info->fullPath.data(), new_file_info->fullPath.length(),
