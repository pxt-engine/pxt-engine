...
namespace pxt {
    ...
    Unique<VulkanMesh> VulkanMesh::create(std::vector<Mesh::Vertex>& vertices, std::vector<uint32_t>& indices) {
        Context& context = Application::get().getContext();

        return createUnique<VulkanMesh>(context, vertices, indices);
    }

    VulkanMesh::VulkanMesh(Context& context, std::vector<Mesh::Vertex>& vertices, std::vector<uint32_t>& indices)
        : m_context(context) {
    ...
    void VulkanMesh::createVertexBuffers(std::vector<Mesh::Vertex>& vertices) {
        ...
        VulkanBuffer stagingBuffer{m_context, vertexSize, m_vertexCount, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
        ...
        m_vertexBuffer = createUnique<VulkanBuffer>(
            m_context, vertexSize, m_vertexCount,
    ...
    void VulkanMesh::createIndexBuffers(std::vector<uint32_t>& indices) {
        ...
        VulkanBuffer stagingBuffer{m_context, indexSize, m_indexCount, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
                                   VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT};
        ...
        m_indexBuffer = createUnique<VulkanBuffer>(
            m_context, indexSize, m_indexCount,
