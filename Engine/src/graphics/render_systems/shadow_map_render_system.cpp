...
namespace pxt {
    ...
    void ShadowMapRenderSystem::createUniformBuffers() {
        ...
        for (size_t i = 0; i < m_lightUniformBuffers.size(); i++) {
            m_lightUniformBuffers[i] =
                createUnique<VulkanBuffer>(m_context, sizeof(ShadowUbo), 1, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
    ...
    void ShadowMapRenderSystem::createRenderPass() {
        ...
        m_renderPass = createUnique<RenderPass>(m_context, renderPassCreateInfo, osAttachments[0], osAttachments[1],
    ...
    void ShadowMapRenderSystem::createOffscreenFrameBuffers() {
        ...
        m_shadowCubeMap = createShared<CubeMap>(m_context, m_shadowMapSize, m_offscreenColorFormat,
        ...
        m_depthStencilImageFb =
            createShared<VulkanImage>(m_context, imageCreateInfo, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
        ...
        m_depthStencilImageFb->transitionImageLayoutSingleTimeCmd(
            ...
            VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT, subresourceRange);
        ...
        for (uint32_t i = 0; i < 6; i++) {
            ...
            m_cubeFramebuffers[i] = createUnique<FrameBuffer>(
                m_context, fbufCreateInfo, "ShadowMapRenderSystem Framebuffer for Cube Face " + std::to_string(i),
                m_shadowCubeMap, m_depthStencilImageFb);
        }
        ...
        m_shadowMapDescriptorInfo.sampler = m_shadowCubeMap->getSamplerHandle();
        ...
        for (uint16_t i = 0; i < 6; i++) {
            ...
            m_debugImageDescriptorInfos[i].sampler = m_shadowCubeMap->getSamplerHandle();
        }
    }
    ...
    void ShadowMapRenderSystem::update(FrameInfo& frameInfo, GlobalUbo& ubo) {
        ...
        uboOffscreen.lightOriginModel =
            glm::translate(glm::mat4(1.0f), glm::vec3(-lightPos.x, -lightPos.y, -lightPos.z));
    ...
    void ShadowMapRenderSystem::render(FrameInfo& frameInfo, Renderer& renderer) {
        ...
        for (uint32_t face = 0; face < 6; face++) {
            ...
            renderer.beginRenderPass(frameInfo.commandBuffer, *m_renderPass, this->getCubeFaceFramebuffer(face),
                                     this->getExtent());
