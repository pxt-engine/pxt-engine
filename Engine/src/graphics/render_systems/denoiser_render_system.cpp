...
namespace pxt {
    ...
    void DenoiserRenderSystem::createSharedSampler() {
        ...
        samplerCreateInfo.addressModeU =
            VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE; // Clamp to edge to avoid artifacts on the borders
        ...
        m_imageSamplerNearest = createShared<VulkanSampler>(m_context, samplerCreateInfo);
    }
    ...
    void DenoiserRenderSystem::createImages(VkExtent2D extent) {
        ...
        m_accumulationImage =
            createUnique<VulkanImage>(m_context, imageCreateInfo, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
        imageViewCreateInfo.image = m_accumulationImage->getVkImage();

        m_accumulationImage->createImageView(imageViewCreateInfo).setSampler(m_imageSamplerNearest);
        ...
        m_tempTemporalOutputImage->createImageView(imageViewCreateInfo).setSampler(m_imageSamplerNearest);
        ...
        m_temporalHistoryImage->createImageView(imageViewCreateInfo).setSampler(m_imageSamplerNearest);
    }
    ...
    void DenoiserRenderSystem::createAccumulationPipelineLayout() {
        ...
        if (vkCreatePipelineLayout(m_context.getDevice(), &pipelineLayoutInfo, nullptr,
                                   &m_accumulationPipelineLayout) != VK_SUCCESS) {
    ...
    void DenoiserRenderSystem::createTemporalFilterPipelineLayout() {
        ...
        if (vkCreatePipelineLayout(m_context.getDevice(), &pipelineLayoutInfo, nullptr,
                                   &m_temporalFilterPipelineLayout) != VK_SUCCESS) {
    ...
    void DenoiserRenderSystem::createSpatialFilterPipelineLayout() {
        ...
        if (vkCreatePipelineLayout(m_context.getDevice(), &pipelineLayoutInfo, nullptr,
                                   &m_spatialFilterPipelineLayout) != VK_SUCCESS) {
    ...
    void DenoiserRenderSystem::createAccumulationPipeline(bool useCompiledSpirvFiles) {
        ...
        const std::string baseShaderPath =
            useCompiledSpirvFiles ? SPV_SHADERS_PATH : SHADERS_PATH + "raytracing/denoising/";
        ...
        m_accumulationPipeline = createUnique<Pipeline>(m_context, shaderFilePath, pipelineConfig);
    }
    ...
    void DenoiserRenderSystem::createTemporalFilterPipeline(bool useCompiledSpirvFiles) {
        ...
        m_temporalFilterPipeline = createUnique<Pipeline>(m_context, shaderFilePath, pipelineConfig);
    }
    ...
    void DenoiserRenderSystem::createSpatialFilterPipeline(bool useCompiledSpirvFiles) {
        ...
        m_spatialFilterPipeline = createUnique<Pipeline>(m_context, shaderFilePath, pipelineConfig);
    }
    ...
    void DenoiserRenderSystem::denoise(FrameInfo& frameInfo, Shared<VulkanImage> sceneImage) {
        ...
        m_accumulationImage->transitionImageLayout(commandBuffer,
        ...
        // Inputs: m_accumulationImage (from Pass 1), m_temporalHistoryImage (previous frame's final output),
        // newFrameImageInfo (raw) Output: m_tempTemporalOutputImage

        m_accumulationImage->transitionImageLayout(commandBuffer, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
                                                   VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                                                   VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT);
        ...
        m_tempTemporalOutputImage->transitionImageLayout(commandBuffer,
        ...
        m_temporalHistoryImage->transitionImageLayout(commandBuffer,
                                                      VK_IMAGE_LAYOUT_GENERAL, // General layout for storage
                                                      VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                                                      VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT);
    ...
    void DenoiserRenderSystem::updateUi() {
        ...
        ImGui::DragInt("Spatial Kernel Radius", reinterpret_cast<int*>(&m_spatialKernelRadius), 1.0f, 0, 10, "%d",
                       ImGuiSliderFlags_AlwaysClamp);
        ImGui::DragFloat("Spatial Gaussian Space SD", &m_spatialSigmaSpace, 0.05f, 0.05f, 10.0f, "%.2f",
                         ImGuiSliderFlags_AlwaysClamp);
        ImGui::DragFloat("Spatial Gaussian Color SD", &m_spatialSigmaColor, 0.05f, 0.05f, 10.0f, "%.2f",
                         ImGuiSliderFlags_AlwaysClamp);
    }

    void DenoiserRenderSystem::copyDenoisedIntoSceneImage(VkCommandBuffer commandBuffer,
                                                          Shared<VulkanImage> sceneImage) {
        // Transition the denoised image to transfer source layout
        m_temporalHistoryImage->transitionImageLayout(commandBuffer, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                                                      VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                                                      VK_PIPELINE_STAGE_TRANSFER_BIT);
        // Transition the scene image to transfer destination layout
        sceneImage->transitionImageLayout(commandBuffer, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
        ...
        vkCmdCopyImage(commandBuffer, m_temporalHistoryImage->getVkImage(), VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                       sceneImage->getVkImage(), VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &copyRegion);
    }
