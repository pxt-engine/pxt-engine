...
namespace pxt {
    ...
    void RayTracingRenderSystem::createPipeline(bool useCompiledSpirvFiles) {
        ...
        m_pipeline = createUnique<Pipeline>(m_context, pipelineConfig);
    }

    // Helper function to align values
    inline uint32_t alignUp(uint32_t value, uint32_t alignment) { return (value + alignment - 1) & ~(alignment - 1); }
    ...
    void RayTracingRenderSystem::createShaderBindingTable() {
        ...
        if (vkGetRayTracingShaderGroupHandlesKHR(m_context.getDevice(), m_pipeline->getHandle(), 0, numGroups,
                                                 rawHandlesDataSize, rawHandles.data()) != VK_SUCCESS) {
        ...
        VulkanBuffer stagingBuffer{m_context, sbtSize, 1, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
                                   VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT};
        ...
        m_sbtBuffer =
            createUnique<VulkanBuffer>(m_context, sbtSize, 1,
        ...
        m_raygenRegion.size =
            rayGenSectionSize; // (https://docs.vulkan.org/spec/latest/chapters/raytracing.html#_ray_generation_shaders)
    ...
    void RayTracingRenderSystem::update(FrameInfo& frameInfo) {
        ...
        m_sceneImage->transitionImageLayout(frameInfo.commandBuffer, VK_IMAGE_LAYOUT_GENERAL,
    ...
    void RayTracingRenderSystem::render(FrameInfo& frameInfo, Renderer& renderer) {
        ...
        vkCmdPushConstants(frameInfo.commandBuffer, m_pipelineLayout,
                           ...
                           sizeof(RayTracingPushConstantData), &pushConstants);

        vkCmdTraceRaysKHR(frameInfo.commandBuffer, &m_raygenRegion, &m_missRegion, &m_hitRegion, &m_callableRegion,
                          m_sceneImage->getWidth(), m_sceneImage->getHeight(), 1);
    }
    ...
    void RayTracingRenderSystem::transitionImageToShaderReadOnlyOptimal(FrameInfo& frameInfo,
        ...
        m_sceneImage->transitionImageLayout(frameInfo.commandBuffer, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
                                            prevStage, nextStage);
    }
    ...
    void RayTracingRenderSystem::updateUi() {
        ...
        if (m_noiseType == 1) {
            ...
            ImGui::Checkbox("Select Single Blue Noise Textures (for Debug)",
                            reinterpret_cast<bool*>(&m_selectSingleBlueNoiseTextures));
            if (m_selectSingleBlueNoiseTextures) {
                std::string inputMessage =
                    "Blue Noise Texture Index (0 to " + std::to_string(BLUE_NOISE_TEXTURE_COUNT - 1) + ")";
