...
namespace pxt {
    ...
    void RayTracingSceneManagerSystem::createTLAS(FrameInfo& frameInfo) {
        ...
        stagingBuffer = createUnique<VulkanBuffer>(
            m_context, sizeof(VkAccelerationStructureInstanceKHR), instanceCount, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
        ...
        instanceBuffer =
            createUnique<VulkanBuffer>(m_context, sizeof(VkAccelerationStructureInstanceKHR), instanceCount,
        ...
        vkGetAccelerationStructureBuildSizesKHR(m_context.getDevice(), VK_ACCELERATION_STRUCTURE_BUILD_TYPE_DEVICE_KHR,
                                                &buildInfo,
                                                &numInstances, // Pointer to the number of primitives (instances)
                                                &m_buildSizeInfo);

        // 4. Allocate BLAS Buffer and Scratch Buffer
        m_tlasBuffer = createUnique<VulkanBuffer>(m_context, m_buildSizeInfo.accelerationStructureSize, 1,
                                                  VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_STORAGE_BIT_KHR |
                                                      VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT,
                                                  VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);

        Unique<VulkanBuffer> scratchBuffer =
            createUnique<VulkanBuffer>(m_context, m_buildSizeInfo.buildScratchSize, 1,
        ...
        memoryBarrier.srcAccessMask =
            ...
            VK_ACCESS_ACCELERATION_STRUCTURE_WRITE_BIT_KHR; // Instance buffer copy + BLAS builds
        memoryBarrier.dstAccessMask =
            VK_ACCESS_ACCELERATION_STRUCTURE_READ_BIT_KHR; // TLAS build reads instance buffer & BLASes
