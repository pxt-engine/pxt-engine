...
namespace pxt {
	...
	ObjectPickingSystem::ObjectPickingSystem(
		...
		m_sceneExtent{ sceneImageExtent }
	{
		createRenderPass();
	...
	void ObjectPickingSystem::createSceneColorIdsImage() {
		...
		m_sceneWithColorIds = createShared<VulkanImage>(
			m_context,
			sceneImageInfo,
	...
	void ObjectPickingSystem::createOffscreenFrameBuffer() {
		...
		std::array<VkImageView, 2> attachments = { m_sceneWithColorIds->getImageView(), m_offscreenDepthImage->getImageView() };
		...
		m_offscreenFb = createUnique<FrameBuffer>(
			m_context,
			framebufferInfo,
			"ObjectPickingSystem Offscreen Framebuffer",
			m_sceneWithColorIds,
	...
	void ObjectPickingSystem::render(FrameInfo& frameInfo, Renderer& renderer, u32vec2 mousePixelCoords) {
		...
		renderer.beginRenderPass(frameInfo.commandBuffer, *m_offscreenRenderPass, *m_offscreenFb, m_sceneExtent, blackClearColor);
		...
		for (auto entity : view) {
			...
			const auto& [transform, meshComponent, objPickingIdComponent] = view.get<TransformComponent, MeshComponent, ObjPickingIdComponent>(entity);
			...
			vkCmdPushConstants(
				...
				0,
				sizeof(ObjPickingPushConstantData),
				&push);
		...
		region.imageOffset = { static_cast<int32_t>(mousePixelCoords.x), static_cast<int32_t>(mousePixelCoords.y), 0 };   // pixel coordinates
		region.imageExtent = { 1, 1, 1 };

		vkCmdCopyImageToBuffer(frameInfo.commandBuffer,
			m_sceneWithColorIds->getVkImage(),
			VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
			m_selectedPixelColorBuffer->getBuffer(),
			1,
			&region);
	}
