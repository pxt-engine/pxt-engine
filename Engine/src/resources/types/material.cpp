...
namespace pxt {
    Material::Material(const glm::vec4& albedoColor, const Shared<Image>& albedoMap, const Shared<Image>& normalMap,
                       const float metallic, const Shared<Image>& metallicMap, const float roughness,
                       const Shared<Image>& roughnessMap, const Shared<Image>& ambientOcclusionMap,
                       const glm::vec4& emissiveColor, const Shared<Image>& emissiveMap, const float transmission,
                       const float ior, const float blinnPhongSpecularIntensity,
                       const float blinnPhongSpecularShininess)
        : m_albedoColor(albedoColor), m_albedoMap(albedoMap), m_normalMap(normalMap), m_metallic(metallic),
          m_metallicMap(metallicMap), m_roughness(roughness), m_roughnessMap(roughnessMap),
          m_ambientOcclusionMap(ambientOcclusionMap), m_emissiveColor(emissiveColor), m_emissiveMap(emissiveMap),
          m_transmission(transmission), m_ior(ior), m_blinnPhongSpecularIntensity(blinnPhongSpecularIntensity),
          m_blinnPhongSpecularShininess(blinnPhongSpecularShininess) {}

    Material::Type Material::getStaticType() { return Type::Material; }

    Material::Type Material::getType() const { return Type::Material; }
    ...
    bool Material::isEmissive() { return m_emissiveColor.a > 0.0f; }
    ...
    Shared<Material> Material::Builder::build() {
        ...
        return createShared<Material>(m_albedoColor, m_albedoMap, m_normalMap, m_metallic, m_metallicMap, m_roughness,
                                      ...
                                      m_transmission, m_ior, m_blinnPhongSpecularIntensity,
    ...
    void Material::drawMaterialUi() {
        ...
        if (ImGui::TreeNode("Textures (Work in progress)")) {
            ...
            if (m_albedoMap) {
                ImGui::Text("Albedo Map is set (id: %s, alias: %s)", m_albedoMap->id.toString().c_str(),
                            m_albedoMap->alias.c_str());
            }

            if (m_normalMap) {
                ImGui::Text("Normal Map is set (id: %s, alias: %s)", m_normalMap->id.toString().c_str(),
                            m_normalMap->alias.c_str());
            }

            if (m_metallicMap) {
                ImGui::Text("Metallic Map is set (id: %s, alias: %s)", m_metallicMap->id.toString().c_str(),
                            m_metallicMap->alias.c_str());
            }

            if (m_roughnessMap) {
                ImGui::Text("Roughness Map is set (id: %s, alias: %s)", m_roughnessMap->id.toString().c_str(),
                            m_roughnessMap->alias.c_str());
            }

            if (m_emissiveMap) {
                ImGui::Text("Emissive Map is set (id: %s, alias: %s)", m_emissiveMap->id.toString().c_str(),
                            m_emissiveMap->alias.c_str());
            }
